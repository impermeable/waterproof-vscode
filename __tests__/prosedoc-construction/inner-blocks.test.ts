import { typeguards } from "waterproof-editor/document";
...
test("Inner coq blocks", () => {
    ...
    expect(blocks.length).toBe(2);
    expect(typeguards.isCoqCodeBlock(blocks[0])).toBe(true);
    ...
    expect(typeguards.isCoqMarkdownBlock(blocks[1].innerBlocks![0])).toBe(true);
});
...
test("Inner Coqdoc blocks", () => {
    ...
    expect(blocks.length).toBe(2);
    expect(typeguards.isCoqMarkdownBlock(blocks[0])).toBe(true);
