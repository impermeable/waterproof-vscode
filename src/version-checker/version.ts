...
export class Version {
  ...
  constructor(
    major: VersionIdentifier,
    minor: VersionIdentifier,
  ...
  static fromString(version: string): Version {
    ...
    if (dotCount < 0 || dotCount > 2)
      throw Error(`Unknown version system encountered '${version}.'`);
    switch (dotCount) {
      case 0:
    ...
    switch (mode) {
      case COMPARE_MODE.GREATER_THAN_EQUALS:
        return new VersionRequirement(
          Version.fromString(rest),
          COMPARE_MODE.GREATER_THAN_EQUALS,
        );
      case COMPARE_MODE.STRICT_EQUALS:
        return new VersionRequirement(
          Version.fromString(rest),
          COMPARE_MODE.STRICT_EQUALS,
        );
      default:
