...
function modifyState(
  f: (previousState: PersistentInfoviewState) => PersistentInfoviewState,
) {
...
if (div && script) {
  const imports = {
    "@leanprover/infoview": script.getAttribute(
      "data-importmap-leanprover-infoview",
    )!,
    react: script.getAttribute("data-importmap-react")!,
    "react/jsx-runtime": script.getAttribute(
      "data-importmap-react-jsx-runtime",
    )!,
    "react-dom": script.getAttribute("data-importmap-react-dom")!,
  };
  loadRenderInfoview(imports, [editorApi, div], async (api) => {
    const previousState: PersistentInfoviewState | undefined =
      vscodeApi.getState();
    ...
    if (previousState !== undefined) {
      ...
      if (previousState.diags !== undefined) {
        await api.gotServerNotification(
