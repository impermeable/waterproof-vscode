...
import {
  RocqGoalAnswer,
  RocqGoalRequest,
  RocqServerStatusToServerStatus,
  GoalRequest,
  PpString,
} from "../../../lib/types";
import { MessageType } from "../../../shared";
import {
  rocqFileProgressNotificationType,
  rocqGoalRequestType,
  rocqServerStatusNotificationType,
  executionInformationNotificationType,
} from "./requestTypes";
...
export class RocqLspClient extends LspClient<
  RocqGoalRequest,
  RocqGoalAnswer<PpString>
> {
  language = "rocq";
  ...
  constructor(
    ...
    this.disposables.push(
      this.client.onNotification(rocqFileProgressNotificationType, (params) => {
    ...
    this.disposables.push(
      this.client.onNotification(rocqServerStatusNotificationType, (params) => {
        const document = this.activeDocument;
        if (!document) return;
  ...
  async prelaunchChecks(): Promise<string[]> {
    if (
      extensions.getExtension("ejgallego.coq-lsp") ||
      extensions.getExtension("maximedenes.vscoq")
    ) {
      wpl.log(
        "Conflicting Rocq extension detected, skipping Waterproof Rocq client.",
      );
      return [];
    }

    const requiredCoqLspVersion = WaterproofPackageJSON.requiredCoqLspVersion(
      this.context,
    );
  ...
  async requestGoals(
    params?: GoalRequest | Position,
  ): Promise<RocqGoalAnswer<PpString>> {
  ...
  getBeginningOfCurrentSentence(): Position | undefined {
    ...
    return this.sentenceManager.getBeginningOfSentence(
