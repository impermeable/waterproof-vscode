...
export class WaterproofStatusBar implements IStatusComponent {
  ...
  update(clientsRunning: string[]): void {
    if (clientsRunning.length > 0) {
      ...
      this.item.text =
        "$(check) Waterproof checker (" + clientsRunning.join(", ") + ")";
      this.item.tooltip =
        "Waterproof document checker is running. Click to stop.";
    } else {
      ...
      this.item.tooltip = new MarkdownString(
        "Waterproof document checker is *not* running. Click to start.",
      );
    }
  }
  ...
  failed(_emsg: string) {
    ...
    this.item.tooltip =
      "Waterproof document checker failed to start. Click to retry.";
  }
