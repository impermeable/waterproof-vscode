...
export class InfoProvider implements Disposable {
    ...
    for (const [, [, subscriptions]] of this.serverNotifSubscriptions) {
      for (const h of subscriptions) {
        try {
          ...
          wpl.log(
            `[InfoView] Could not dispose of server notification subscription: ${e}`,
          );
        }
      }
    }
    ...
    for (const [, [, subscriptions]] of this.clientNotifSubscriptions) {
      for (const h of subscriptions) {
        try {
          ...
          wpl.log(
            `[InfoView] Could not dispose of client notification subscription: ${e}`,
          );
        }
      }
    }
  ...
  private subscribeDiagnosticsNotification(
    ...
    const h = client.diagnostics((params) => {
      wpl.debug(
        `[infoProvider] diagnostics→infoview, uri=${params.uri}, count=${params.diagnostics.length}, ranges=${JSON.stringify(params.diagnostics.map((d) => d.range))}`,
      );
  ...
  static filterHypothesesList(hypotheses: any[]): any[] {
    const hyps = [];
    switch (
      WaterproofConfigHelper.get(WaterproofSetting.VisibilityOfHypotheses)
    ) {
      // Keep all hypotheses
      case "all":
      ...
      case "limited":
        for (const hyp of hypotheses) {
          if (
            !(hyp.type?.tag?.[1] !== undefined && "text" in hyp.type.tag[1])
          ) {
            hyps.push(hyp);
          }
        }
  ...
  private editorApi: EditorApi = {
    ...
    subscribeServerNotifications: async (method: string) => {
      ...
      if (method === "textDocument/publishDiagnostics") {
        ...
        subscriptions.push(
          this.subscribeDiagnosticsNotification(this.client, method),
        );
        this.serverNotifSubscriptions.set(method, [1, subscriptions]);
      } else if (method.startsWith("$")) {
        const subscriptions: Disposable[] = [];
        subscriptions.push(
          this.subscribeCustomNotification(this.client, method),
        );
        this.serverNotifSubscriptions.set(method, [1, subscriptions]);
      } else {
        throw new Error(
          `subscription to ${method} server notifications not implemented`,
        );
      }
    },
    unsubscribeServerNotifications: async (method: string) => {
      ...
      if (!el) {
        console.warn(
          `[InfoProvider] unsubscribeServerNotifications: no subscription for ${method}`,
        );
        return;
      }
    ...
    subscribeClientNotifications: async (method: string) => {
      ...
      if (method === "textDocument/didChange") {
        const subscriptions: Disposable[] = [];
        subscriptions.push(
          this.subscribeDidChangeNotification(this.client, method),
        );
        this.clientNotifSubscriptions.set(method, [1, subscriptions]);
      } else if (method === "textDocument/didClose") {
        const subscriptions: Disposable[] = [];
        subscriptions.push(
          this.subscribeDidCloseNotification(this.client, method),
        );
        this.clientNotifSubscriptions.set(method, [1, subscriptions]);
      } else {
        throw new Error(
          `Subscription to '${method}' client notifications not implemented`,
        );
      }
    },
    ...
    applyEdit: async (e: WorkspaceEdit) => {
      ...
      if (!document || !this.client.webviewManager) {
        wpl.log(
          `[InfoProvider] Cannot apply edit: no active document or webviewManager`,
        );
        return;
      }
  ...
  public async sendPosition(loc: Location) {
    wpl.debug(
      `[infoProvider] sendPosition uri=${loc.uri}, pos=${JSON.stringify(loc.range.start)}, apiExists=${!!this.api}`,
    );
