...
test("Multilean wrapper with an input area", () => {
  ...
  const cg = containerBlocks[0];
  const innerInput = cg.innerBlocks.filter((b) =>
    typeguards.isInputAreaBlock(b),
  );
...
test("Multilean wrapper with a math display block", () => {
  ...
  const cg = containerBlocks[0];
  const innerMath = cg.innerBlocks.filter((b) =>
...
test("Typical exercise pattern: code + input + code inside multilean", () => {
  ...
  const innerNewlines = cg.innerBlocks.filter((b) =>
    typeguards.isNewlineBlock(b),
  );
  ...
  const inputInnerCode = innerInput[0].innerBlocks.filter((b) =>
    typeguards.isCodeBlock(b),
  );
...
test("Exercise pattern: code before input only (no trailing code block)", () => {
  ...
  expect(
    innerInput[0].innerBlocks.filter((b) => typeguards.isCodeBlock(b))[0]
      .stringContent,
  ).toBe("  trivial");
});
...
test("Content before and after multilean block", () => {
  ...
  const cg = containerBlocks[0];
  expect(cg.innerBlocks.filter((b) => typeguards.isCodeBlock(b)).length).toBe(
    2,
  );
  expect(
    cg.innerBlocks.filter((b) => typeguards.isInputAreaBlock(b)).length,
  ).toBe(1);
...
test("ContainerBlock stringContent matches the inner document slice", () => {
  ...
  expect(document.substring(cg.innerRange.from, cg.innerRange.to)).toBe(
    cg.stringContent,
  );
});
...
test("Newline between multilean close and following code block is significant (not stripped)", () => {
  ...
  const newlinesBetween = blocksBetween.filter((b) =>
    typeguards.isNewlineBlock(b),
  );
