...
export function getNonInputRegions(doc: string) {
  ...
  const allTags = [...inputOpenTags, ...inputCloseTags]
    .map((value) => {
  ...
  const pairsCombined = Array.from({ length: allTags.length / 2 }, (_, i) => {
  ...
  const outsideInputAreaRegions = Array.from(
    { length: pairsCombined.length + 1 },
    (_, i) => {
      if (i == 0) {
        // special case
        ...
        return {
          from: pairsCombined[i - 1].close.position,
          to: pairsCombined[i].open.position,
        };
      }
    },
  ).map((value) => {
...
export const showRestoreMessage = (cbOnRestore: () => void) => {
  window
    .showErrorMessage(
      "Waterproof: The content outside of the input areas changed, this is not supposed to happen! Click the button below to restore.",
      RESTORE,
    )
    .then(restoreHandler(cbOnRestore));
};
