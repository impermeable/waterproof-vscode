...
describe("setupWaterproofProfile (triggered via checkConflictingExtensions)", () => {
  type ExtEntry = {
    id: string;
    packageJSON: { isBuiltin: boolean; version: string };
  };

  async function triggerSetup(
    installedConflicting: string[],
    allExtensions: ExtEntry[] = [],
    installedAll?: string[],
  ) {
  ...
  function readProfileExtensions(
    data: Uint8Array,
  ): { identifier: { id: string }; disabled?: boolean }[] {
  ...
  it("includes waterproof extension when installed", async () => {
    ...
    await triggerSetup(
      ["leanprover.lean4"],
      [],
      ["leanprover.lean4", "waterproof-tue.waterproof"],
    );
  ...
  it("does not include arbitrary user extensions", async () => {
    ...
    jest.requireMock("vscode").extensions.all = [
      {
        id: "publisher.some-other-ext",
        packageJSON: { isBuiltin: false, version: "1.0.0" },
      },
    ];
