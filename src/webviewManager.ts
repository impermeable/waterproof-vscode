...
class ActiveWebviews {
  ...
  public find(time: number) {
    ...
    while (this._queue[i] >= time) {
      // There may be an issue where a panel supports insert and can send insert messages
      i++;
      if (this._queue.length == i)
        throw new Error("There are no valid active panels");
    }
...
export class WebviewManager extends EventEmitter {
  ...
  private readonly _toolWebviews: Map<string, WaterproofPanel> = new Map<
    string,
    WaterproofPanel
  >();

  // Waterproof webviews, stores the view based on Doc uri
  private readonly _waterproofWebviews: Map<string, WaterproofWebview> =
    new Map<string, WaterproofWebview>();
  ...
  public addToolWebview(name: string, webview: WaterproofPanel) {
    ...
    webview.on(WebviewEvents.change, (state) => {
      if (state == WebviewState.focus && webview.supportInsert)
        this._active.insert(name);
  ...
  public postMessage(panelName: string, message: Message) {
    ...
    else throw new Error(" No such webview available ");
  }
    ...
    switch (message.type) {
      ...
      case MessageType.response: {
        ...
        if (!message.requestId) {
          console.error(
            "Received response message without a requestId!\nThe message:",
            message,
          );
          return;
        }
      ...
      case MessageType.cursorChange: {
        ...
        if (webview.documentIsUpToDate) {
          ...
          wpl.debug(
            `[cursorChange] offset=${offset} → pos=${pos.line}:${pos.character}, docUpToDate=true`,
          );
          ...
          const callback = () => {
            const pos = document.positionAt(offset);
            wpl.debug(
              `[cursorChange] offset=${offset} → pos=${pos.line}:${pos.character}, docUpToDate=true (deferred)`,
            );
            this._lineStatus.update(pos);
            this.emit(WebviewManagerEvents.cursorChange, document, pos);
          };
          if (webview.listeners(WebviewEvents.finishUpdate).length == 0)
            webview.once(WebviewEvents.finishUpdate, callback);
          else
            wpl.debug(
              `[cursorChange] finishUpdate listener already registered, skipping`,
            );
        }
        break;
      }
      case MessageType.applyStepError: {
        const mes =
          "The editor encountered an error in applying an edit (see log for details): " +
          message.body;
      ...
      case MessageType.command:
        ...
        this.onToolsMessage("goals", {
          type: MessageType.command,
          body: { command: "createHelp" },
        });
        setTimeout(
          () =>
            this.onToolsMessage("goals", {
      ...
      case MessageType.viewportHint:
        wpl.debug(
          `[WEBVIEW] ViewportHint message received: start=${message.body.start}, end=${message.body.end}`,
        );
        this.emit(WebviewManagerEvents.viewportHint, {
          document,
          ...message.body,
        });
        break;
      default:
        console.error(
          `Unrecognized message type ${message.type}, not handled by webview manager`,
        );
        break;
    }
  }
    ...
    switch (msg.type) {
      ...
      default:
        console.error(
          "The message type " +
            msg.type +
            " is not currently supported by webview manager",
        );
        break;
    }
  }
