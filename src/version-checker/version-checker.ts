...
export class VersionChecker {
  ...
  constructor(
    ...
    rocqLspVersion: string,
    rocqWaterproofVersion: string,
  ) {
    ...
    this._reqVersionRocqWP = VersionRequirement.fromString(
      rocqWaterproofVersion,
    );
  }
   ...
   * @returns
   */
  public async checkWaterproofLib(): Promise<
    { wpVersion: Version; requiredRocqVersion: Version } | VersionError
  > {
    if (this._wpPath === undefined)
      return { reason: "Waterproof.path is undefined" };
    const ext = process.platform === "win32" ? ".exe" : "";
    const ocamlfindPath = WaterproofFileUtil.join(
      WaterproofFileUtil.getDirectory(this._wpPath),
      `ocamlfind${ext}`,
    );
    ...
    const findlib_conf = WaterproofFileUtil.join(
      WaterproofFileUtil.getDirectory(this._wpPath),
      `findlib.conf`,
    );
    const needEnv =
      getPlatformHelper() === "windows" &&
      this._wpPath !==
        WaterproofPackageJSON.defaultCoqLspPathWindows(this._context);
    ...
    try {
      ...
      return {
        wpVersion: versionRocqWaterproof,
        requiredRocqVersion: versionRequiredRocq,
      };
    } catch (err: unknown) {
  ...
  private informWaterproofLibNotFound() {
    ...
    if (this.platformHasAutoInstaller() && env.remoteName === undefined) {
      window
        ...
        .then(this.handleAutoInstallOrInstructions);
    } else {
      window
        .showInformationMessage(
          message_no_installer,
          { modal: true },
          OPEN_INSTRUCTIONS,
        )
        .then(this.handleAutoInstallOrInstructions);
    ...
    if (platform == "windows" && env.remoteName === undefined) {
      ...
      if (this.platformHasAutoInstaller()) {
        window
          ...
          .then(this.handleDownloadInstaller);
      } else {
        window
          .showErrorMessage(message, { modal: true }, DOWNLOAD_INSTALLER)
          .then(this.handleDownloadInstaller);
      }
    } else {
      ...
      window
        .showErrorMessage(message, { modal: true }, OPEN_README)
        .then(this.handleOpenReadme);
    }
  }

  private handleOpenReadme(value: typeof OPEN_README | undefined) {
    if (value === OPEN_README)
      env.openExternal(
        Uri.parse(
          "https://github.com/impermeable/waterproof-vscode#waterproof",
        ),
      );
  }
   ...
   * @param value -
   */
  private handleDownloadInstaller(
    value: typeof AUTO_INSTALL | typeof DOWNLOAD_INSTALLER | undefined,
  ) {
    if (value === DOWNLOAD_INSTALLER) {
      console.log("DOWNLOAD INSTALLER");
      env.openExternal(
        Uri.parse(
          "https://github.com/impermeable/waterproof-dependencies-installer/releases/",
        ),
      );
    } else if (value === AUTO_INSTALL) {
      commands.executeCommand(
        `workbench.action.openWalkthrough`,
        `waterproof-tue.waterproof#waterproof.auto`,
        false,
      );
    }
  }
   ...
   * @param value -
   */
  private handleAutoInstallOrInstructions(
    value: typeof AUTO_INSTALL | typeof OPEN_INSTRUCTIONS | undefined,
  ) {
