...
export class SentenceManager implements IFileProgressComponent {
  ...
  private getRank(position: Position): number {
    ...
    while (l < r) {
      ...
      if (this.sentenceEndPositions[m].isBefore(position)) l = m + 1;
      else r = m;
    }
    return l;
  }
  ...
  getEndOfSentence(
    ...
    if (i === n) return strict ? undefined : this.sentenceEndPositions[n - 1];
    else return this.sentenceEndPositions[i];
  }
