...
jest.mock(
  "../../../lib/types",
  () => ({ RocqServerStatusToServerStatus: jest.fn() }),
  { virtual: true },
);
jest.mock("../../../src/helpers", () => ({
  ...lspMocks().helpers(),
  WaterproofPackageJSON: {
    requiredCoqLspVersion: jest.fn(),
    requiredCoqWaterproofVersion: jest.fn(),
  },
}));
...
jest.mock("../sentenceManager", () => lspMocks().sentenceManager(), {
  virtual: true,
});
jest.mock("../clientTypes", () => ({}), { virtual: true });
jest.mock("../requestTypes", () => lspMocks().requestTypes(), {
  virtual: true,
});
jest.mock(
  "../qedStatus",
  () => ({ findOccurrences: jest.fn(), areInputAreasValid: jest.fn() }),
  { virtual: true },
);
jest.mock(
  "./requestTypes",
...
describe("RocqLspClient.prelaunchChecks — conflicting extension guard", () => {
  it("skips the Rocq client when Coq LSP is installed", async () => {
    jest
      .mocked(extensions.getExtension)
  ...
  it("logs that a conflicting Rocq extension was found", async () => {
    ...
    expect(wpl.log).toHaveBeenCalledWith(
      expect.stringContaining("Conflicting Rocq extension detected"),
    );
  });

  it("proceeds to the version check when no conflicting extension is installed", async () => {
    const { VersionChecker } =
      await import("../../../src/version-checker/version-checker");
