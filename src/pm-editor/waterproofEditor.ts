...
export class WaterproofEditorProvider
  implements vscode.CustomTextEditorProvider
{
  ...
  public static register(
    ...
    const providerRegistration = vscode.window.registerCustomEditorProvider(
      WaterproofEditorProvider.viewType,
      provider,
      {
        webviewOptions: {
  ...
  public async resolveCustomTextEditor(
    ...
    return WaterproofWebview.createWaterproofView(
      webviewPanel,
      ...
      .catch((reason) => {
        console.error(
          `==> Failed to create editor view. \n==> The reason should be printed below.`,
        );
        console.log(reason);
      });
  }

  public registerHistoryCommandListeners(
    wantsToRegister: WaterproofWebview,
    undoListener: () => void,
    redoListener: () => void,
  ) {
    if (!this.registeredHistoryCommands) {
      this._context.subscriptions.push(
        (this._undoCommand = vscode.commands.registerCommand(
          "undo",
          undoListener,
        )),
        (this._redoCommand = vscode.commands.registerCommand(
          "redo",
          redoListener,
        )),
      );
  ...
  public disposeHistoryCommandListeners(wantsToDispose?: WaterproofWebview) {
    if (
      wantsToDispose === undefined ||
      this._lastRegistered == wantsToDispose
    ) {
