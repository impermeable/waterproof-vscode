...
test("Expect comment to be pretty-printed", () => {
    expect(translateRocqDoc("This is just a pretty-printed comment.")).toBe(`This is just a pretty-printed comment.`);
});

test("Expect whitespace to still work", () => {
    expect(translateRocqDoc("        This is just a comment.     ")).toBe(`        This is just a comment.     `);
});
...
test("Expect default pretty printing character to be replaced", () => {
    expect(translateRocqDoc("Here follows a pretty-printing character: ->")).toBe(`Here follows a pretty-printing character: →`);
});
...
test("Expect quoted rocq to be replaced", () => {
    expect(translateRocqDoc("[let id := 3] and [3 : nat]")).toBe("`let id := 3` and `3 : nat`");
});

test("Expect quoted rocq to be replaced and behave nicely with nested square brackets", () => {
    expect(translateRocqDoc("[let id := fun [T : Type] (x : t) => x in id 0]")).toBe("`let id := fun [T : Type] (x : t) ⇒ x in id 0`");
});

test("Expect quoted rocq to be replaced and behave nicely with nested square brackets 2", () => {
    expect(translateRocqDoc("[let id := fun [T : Type] (x : t) => x in id 0] and [bool]")).toBe("`let id := fun [T : Type] (x : t) ⇒ x in id 0` and `bool`");
});
...
test("Preserves whitespace inside rocq code cell.", () => {
    expect(translateRocqDoc("This is a rocq code cell\n\nHello whitespace.   \n\n   \n")).toBe(`This is a rocq code cell\n\nHello whitespace.   \n\n   \n`);
});

test("From indented list in Rocqdoc comments, make markdown list", () => {

    expect(translateRocqDoc("- First item\n- Second item\n  - Indented item\n  - Second indented item\n- Third item"))
    .toBe(`- First item\n- Second item\n    - Indented item\n    - Second indented item\n- Third item`);
});

test("Replace % inside of rocqdoc", () => {
    expect(rocqdocToMarkdown("%\\text{coqdoc in mathinline?!}%")).toBe("<math-inline>\\text{coqdoc in mathinline?!}</math-inline>");
});