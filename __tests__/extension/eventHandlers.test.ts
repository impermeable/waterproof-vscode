...
jest.mock("vscode", () => ({
    ...
    commands: { registerCommand: jest.fn(), registerTextEditorCommand: jest.fn(), executeCommand: jest.fn() },
...
jest.mock("../../src/helpers", () => ({
    ...
    WaterproofConfigHelper: { get: jest.fn(), update: jest.fn(), configuration: {} },
    WaterproofFileUtil: {},
...
jest.mock("../../src/pm-editor", () => ({ WaterproofEditorProvider: { register: jest.fn() } }));
jest.mock("../../src/util", () => ({
    checkConflictingExtensions: jest.fn(),
    excludeRocqFileTypes: jest.fn(),
    checkTrimmingWhitespace: jest.fn(),
}));
jest.mock("../../src/components/enableButton", () => ({ WaterproofStatusBar: class {} }));
jest.mock("../../src/webviews/sidePanel", () => ({ addSidePanel: jest.fn(), SidePanelProvider: class {} }));
jest.mock("../../src/webviews/standardviews/search", () => ({ Search: class {} }));
jest.mock("../../src/webviews/standardviews/execute", () => ({ ExecutePanel: class {} }));
jest.mock("../../src/webviews/standardviews/symbols", () => ({ SymbolsPanel: class {} }));
jest.mock("../../src/webviews/standardviews/tactics", () => ({ TacticsPanel: class {} }));
jest.mock("../../src/webviews/goalviews/debug", () => ({ DebugPanel: class {} }));
jest.mock("../../src/webviews/goalviews/goalsPanel", () => ({ GoalsPanel: class {} }));
jest.mock("../../src/webviews/goalviews/compositeGoalsPanel", () => ({ CompositeGoalsPanel: class {} }));
jest.mock("../../src/lsp-client/composite", () => ({ CompositeClient: class {} }));
jest.mock("../../src/lsp-client/rocq", () => ({ RocqLspServerConfig: { create: jest.fn() } }));
jest.mock("../../src/lsp-client/lean", () => ({ LeanLspServerConfig: { create: jest.fn() } }));
jest.mock("../../src/helpers/exerciseSheet", () => ({ clearInputCells: jest.fn() }));
...
const proto = Waterproof.prototype as unknown as {
    ...
    onViewportHint: (this: unknown, p: { document: FakeDoc; start: number; end: number }) => void;
    onFocus: (this: unknown, d: FakeDoc) => Promise<void>;
    onCursorChange: (this: unknown, d: FakeDoc, p: unknown) => void;
    onCommand: (this: unknown, source: { setResults: jest.Mock }, command: string) => void;
};
...
describe("onCommand", () => {
    ...
    it("runs an ordinary command and forwards its results", async () => {
        ...
        expect(executeCommandMock).toHaveBeenCalledWith(ctx.client.rocqClient, "Check nat.");
