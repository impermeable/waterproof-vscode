...
test("NewlineBlock 2", () => {
  ...
  expect(b2.range).toStrictEqual<BlockRange>({
    from: input.length - 1,
    to: input.length,
  });
  expect(b2.innerRange).toStrictEqual<BlockRange>({
    from: input.length - 1,
    to: input.length,
  });
});
...
test("NewlineBlock 3", () => {
  ...
  expect(b3.range).toStrictEqual<BlockRange>({
    from: input.length - 1,
    to: input.length,
  });
  expect(b3.innerRange).toStrictEqual<BlockRange>({
    from: input.length - 1,
    to: input.length,
  });
});
