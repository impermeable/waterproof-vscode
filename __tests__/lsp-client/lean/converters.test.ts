...
jest.mock("vscode-languageclient/lib/common/utils/async", () => ({
...
import { Code2ProtocolConverter, DiagnosticSeverity, Protocol2CodeConverter } from "vscode-languageclient";
...
interface TestP2C {
    ...
    asDiagnostics: (diags: TestDiagnostic[], token?: unknown) => Promise<TestDiagnostic[]>;
...
function makeP2C(overrides = {}): TestP2C & Protocol2CodeConverter {
    const base: TestP2C = {
        asDiagnostic: jest.fn((protDiag: TestDiagnostic): TestDiagnostic => ({
            message:  protDiag.message,
            severity: protDiag.severity,
            range:    protDiag.range,
        })),
        asDiagnostics: jest.fn(async (diags: TestDiagnostic[]): Promise<TestDiagnostic[]> =>
            diags.map(d => ({ message: d.message, severity: d.severity, range: d.range }))
        ),
...
function makeC2P(overrides = {}): TestC2P & Code2ProtocolConverter {
    const base: TestC2P = {
        asDiagnostic: jest.fn((diag: TestDiagnostic): TestDiagnostic => ({
...
describe("patchDiagnosticConverters", () => {
    ...
    describe("p2c.asDiagnostics (patched)", () => {
        ...
        it("maps each item through the patched asDiagnostic", async () => {
            ...
            const tags = [1];
            const diags = [
                makeDiag({ message: "a", leanTags: tags, fullRange: FULL_RANGE, isSilent: false }),
                makeDiag({ message: "b", severity: DiagnosticSeverity.Warning, leanTags: [], fullRange: FULL_RANGE, isSilent: true }),
            ];
    ...
    describe("c2p.asDiagnostics (patched)", () => {
        ...
        it("maps each item through the patched asDiagnostic", async () => {
            ...
            const diags = [
                makeDiag({ message: "x", leanTags: [1], fullRange: FULL_RANGE, isSilent: false }),
                makeDiag({ message: "y", leanTags: [],  fullRange: FULL_RANGE, isSilent: true  }),
            ];
