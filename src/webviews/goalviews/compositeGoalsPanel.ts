...
export class CompositeGoalsPanel implements IGoalsComponent {
  ...
  constructor(protected readonly panel: GoalsPanel) {
  ...
  async updateGoals(client: CompositeClient): Promise<void> {
    wpl.debug(
      `[compositeGoals] updateGoals called, panelOpen=${this.panel.isOpened}, clientExists=${!!client}`,
    );
    ...
    if (client.activeClient instanceof LeanLspClient) {
      ...
      const cursorStr = cursorPos
        ? `${cursorPos.line}:${cursorPos.character}`
        : "undefined";
      wpl.debug(
        `[compositeGoals] Lean path, lastState=${this.lastState}, cursorPos=${cursorStr}, activeDoc=${client.activeDocument?.uri.toString().split("/").pop()}`,
      );
      ...
      try {
        ...
        if (this.infoProvider === undefined) {
          wpl.debug(
            `[compositeGoals] creating new InfoProvider, initInfoview at ${JSON.stringify(loc.range.start)}`,
          );
          this.infoProvider = new InfoProvider(client.activeClient, this.panel);
          this.infoProvider.initInfoview(loc);
        } else {
          wpl.debug(
            `[compositeGoals] sendPosition at ${JSON.stringify(loc.range.start)}`,
          );
