...
} from "./lsp-client/commandExecutor";
...
export class Waterproof implements Disposable {
  ...
  constructor(
    ...
    this.webviewManager.on(
      ...
      this.onEditorReady.bind(this),
    );
    this.webviewManager.on(
      WebviewManagerEvents.viewportHint,
      this.onViewportHint.bind(this),
    );
    ...
    this.disposables.push(
      WaterproofEditorProvider.register(context, this.webviewManager),
    );
    ...
    this.registerCommand("defaultPath", () => {
      ...
      switch (process.platform) {
        ...
        // WINDOWS
        case "win32":
          defaultValue = WaterproofPackageJSON.defaultCoqLspPathWindows(
            this.context,
          );
          break;
        case "cygwin":
    ...
    this.registerCommand("setArgsWindowsBasedOnCoqLspPath", () => {
      ...
      if (coqLspPath !== "coq-lsp") {
        const coqlib = WaterproofFileUtil.join(
          WaterproofFileUtil.getDirectory(coqLspPath),
          "..\\lib\\coq",
        );
        const findlib_config = WaterproofFileUtil.join(
          WaterproofFileUtil.getDirectory(coqLspPath),
          "findlib.conf",
        );
        WaterproofConfigHelper.update(
          WaterproofSetting.Args,
          [`--coqlib=${coqlib}`, `--findlib_config=${findlib_config}`],
          ConfigurationTarget.Global,
        );
      }
    });
    ...
    this.registerCommand("autoInstall", async () => {
      ...
      const downloadLink = WaterproofPackageJSON.installerDownloadLinkWindows(
        this.context,
      );
      ...
      const uninstallerLocation = WaterproofFileUtil.join(
        WaterproofFileUtil.getDirectory(
  ...
  private async onFocus(document: TextDocument): Promise<void> {
    ...
    if (
      this.client.activeDocument?.uri.toString() !== document.uri.toString()
    ) {
  ...
  private onCursorChange(document: TextDocument, position: Position): void {
    wpl.debug(
      `[ext:cursorChange] pos=${position.line}:${position.character}, doc=${document.uri.toString().split("/").pop()}`,
    );
  ...
  public async help(): Promise<Array<string>> {
    // Execute command
    const wpHelpResponse = await executeCommandFullOutput(
  ...
  public async proofContext(cursorMarker: string = "{!* CURSOR *!}"): Promise<{
    ...
    const startProof = document.offsetAt(
      new Position(
        firstBefore.range.start.line,
        firstBefore.range.start.character,
      ),
    );
    ...
    return {
      ...
      withCursorMarker: removeMarkersAndWhitespace(
        theProof.substring(0, offsetIntoMatch) +
          cursorMarker +
  ...
  async exportExerciseSheet() {
    ...
    try {
      if (document) {
        ...
        const fileExtension: string = fileName.substring(
  ...
  async initializeClient(): Promise<void> {
    ...
    checkConflictingExtensions(this.context).catch((err) =>
      wpl.log(`Conflict check failed: ${err}`),
    );
    ...
    if (skipLaunchChecksSetting !== "none") {
      allowedLanguages =
        skipLaunchChecksSetting === "all"
          ? ["rocq", "lean4"]
          : [skipLaunchChecksSetting];
    ...
    return this.client
      .startWithHandlers(this.webviewManager, allowedLanguages)
      .then(
        (clients) => {
          this.webviewManager.open("goals");
  ...
  private async updateGoals(
    ...
    wpl.debug(
      `[ext:updateGoals] pos=${position.line}:${position.character}, doc=${document.uri.toString().split("/").pop()}`,
    );
    if (!this.client.isRunning()) {
